  ...
  methods: {
    async fetchCartData() {
      try {
        ...
        if (user && user.id) {
                ...
                try {
                  if (
                    ...
                    const imageArray = JSON.parse(
                      item.productId.image.replace(/'/g, '"')
                    );
          ...
          if (guestCartId) {
            const guestCartData =
              await CartService.getGuestCartItemsWithDetails(guestCartId);
    ...
    async createOrderWithPayment() {
      try {
        ...
        if (
          orderResponse.data &&
          (orderResponse.data.success || orderResponse.data._id)
        ) {
          // 2. Lấy orderId từ response
          console.log(
            "Chi tiết response tạo đơn hàng:",
          ...
          eventBus.emit("show-alert", {
            ...
            message:
              "Không thể tạo liên kết thanh toán MoMo. Vui lòng thử lại hoặc chọn phương thức thanh toán khác.",
            autoClose: true,
          });
        }
      } catch (error) {
        ...
        eventBus.emit("show-alert", {
          ...
          message:
            "Có lỗi xảy ra khi tạo đơn hàng hoặc thanh toán, vui lòng thử lại sau",
          autoClose: true,
        });
      } finally {
        this.loading = false;
    ...
    async createOrderWithoutPayment() {
      try {
        ...
        console.log(
          "Đang tạo đơn hàng chưa thanh toán với dữ liệu:",
          orderData
        );
    ...
    proceedToCheckout() {
      if (this.cartItems.length === 0) {
        eventBus.emit("show-alert", {
          ...
          message:
            "Giỏ hàng của bạn đang trống hãy mua sắm để mang về những sản phẩm bổ ích và thú vị.",
          autoClose: true,
          duration: 5000,
          textAlign: "center",
        });
...
.checkout-summary-container {
  ...
  background-color: rgb(
    ...
    235,
    225
  ); /* Thêm màu nền để che phần content phía dưới */
